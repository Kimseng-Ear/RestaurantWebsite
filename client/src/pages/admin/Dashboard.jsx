   ...
   return (
      <div className="min-h-screen bg-[#FDFDFD] flex font-sans text-slate-700 selection:bg-earth-900 selection:text-white">
         ...
         <motion.aside
            ...
            className="bg-white border-r border-slate-100 hidden md:flex flex-col sticky top-0 h-screen transition-all shadow-[10px_0_40px_-20px_rgba(0,0,0,0.03)] z-40"
         >
         ...
         {/* Main Content */}
         <main className="flex-grow flex flex-col min-h-screen overflow-x-hidden pt-4 md:pt-0">
            {/* Navigation Bar */}
            <header className="h-24 bg-white/80 backdrop-blur-xl border-b border-slate-100 sticky top-0 z-30 flex items-center justify-between px-8 lg:px-12">
               <div className="flex items-center gap-6">
                  <button
                     onClick={() => setIsSidebarOpen(!isSidebarOpen)}
                     className="p-3 bg-slate-50 hover:bg-white border border-slate-100 rounded-2xl text-slate-400 hover:text-earth-900 transition-all hover:shadow-sm"
                  >
                     <MenuIcon size={20} />
                  </button>
                  <div className="hidden lg:block">
                     <h1 className="text-2xl font-black text-earth-900 tracking-tight">{activeTab}</h1>
                     <p className="text-xs text-slate-400 font-medium tracking-wide">Managing Leisure Lake Digital Estate</p>
                  </div>
               </div>
            ...
            <div className="p-8 lg:p-12 overflow-y-auto flex-grow max-w-[100rem]">
               <AnimatePresence mode="wait">
                     ...
                     <motion.div initial={{ opacity: 0, scale: 0.98 }} animate={{ opacity: 1, scale: 1 }} exit={{ opacity: 0, scale: 0.98 }} transition={{ duration: 0.4, ease: easing }} key="overview" className="space-y-10">
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                           {[
                              ...
                              <motion.div key={i} initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: i * 0.1, ease: easing }}
                                 className="bg-white rounded-[2.5rem] p-8 border border-slate-100 shadow-sm hover:shadow-xl transition-all duration-500 group relative overflow-hidden flex items-center gap-6">
                        ...
                        <div className="grid grid-cols-1 xl:grid-cols-3 gap-8">
                           ...
                           <div className="space-y-8">
                              ...
                              {/* Rating Summary */}
                              <div className="bg-earth-900 text-white rounded-[3rem] p-10 relative overflow-hidden">
                  ...
                  {activeTab === 'Reservations' && (
                     <motion.div initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} key="reservations" className="space-y-10">
                        <div className="flex flex-col lg:flex-row items-center justify-between gap-8 bg-white p-8 rounded-[3rem] border border-slate-100">
                           <div className="flex items-center gap-6 w-full lg:w-auto">
                              <div className="relative flex-grow lg:flex-none lg:w-96">
                           ...
                           <button className="flex items-center gap-3 bg-earth-900 text-white px-8 py-4 rounded-2xl font-black text-[10px] uppercase tracking-widest hover:shadow-xl hover:shadow-earth-900/20 active:scale-95 transition-all w-full lg:w-auto">
                     ...
                     <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="space-y-12">
                        <div className="flex flex-col md:flex-row items-center justify-between gap-6">
                           ...
                           <button
                              ...
                              className="bg-earth-900 text-white px-10 py-5 rounded-2xl font-black uppercase text-[10px] tracking-[0.3em] flex items-center gap-4 hover:shadow-2xl hover:shadow-earth-900/40 transition-all active:scale-95"
                           >
                              <Plus size={18} strokeWidth={3} /> Launch New Dish
                           </button>
                        </div>
                        <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-10">
                           {menu.map(dish => <DishCard key={dish._id} dish={dish} onEdit={(d) => { setEditingItem(d); setMenuForm(d); setIsMenuModalOpen(true); }} onDelete={fetchData} onFeature={fetchData} />)}
                        </div>
                     </motion.div>
                  )}
                     ...
                     <motion.div initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} className="space-y-12">
                        <div className="flex flex-col md:flex-row items-center justify-between gap-6">
                           ...
                           <button
                              ...
                              className="bg-earth-900 text-white px-10 py-5 rounded-2xl font-black uppercase text-[10px] tracking-[0.3em] flex items-center gap-4 hover:shadow-2xl hover:shadow-earth-900/40 transition-all active:scale-95"
                           >
                              <Plus size={18} strokeWidth={3} /> Upload Visual Asset
                           </button>
                        </div>
                        <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-10">
                           {gallery.map(img => (
                              <div key={img._id} className="bg-white rounded-[3rem] overflow-hidden border border-slate-100 shadow-sm group hover:shadow-2xl transition-all duration-700 relative flex flex-col">
                                 ...
                                 <div className="p-10 flex flex-col flex-grow">
                                    <h4 className="text-2xl font-black text-earth-900 tracking-tighter mb-4">{img.title || 'Untitled Narrative'}</h4>
                        ...
                        <div className="bg-white rounded-[4rem] border border-slate-100 overflow-hidden shadow-sm">
                           <div className="overflow-x-auto">
                              <table className="w-full text-left">
                                 <thead className="bg-slate-50 border-b border-slate-100">
                                    <tr className="text-[10px] font-black text-slate-400 uppercase tracking-[0.2em]">
                                       <th className="px-12 py-10">Guest Identity</th>
                                       <th className="px-6 py-10">Sensory Rating</th>
                                       <th className="px-6 py-10">Narrative Comment</th>
                                       <th className="px-12 py-10 text-right">Moderation</th>
                                    </tr>
                                 </thead>
                  ...
                  {/* ⚙️ SETTINGS TAB */}
                  {activeTab === 'Settings' && (
                     <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} key="settings" className="max-w-4xl mx-auto py-8">
                        <div className="bg-white p-12 lg:p-16 rounded-[4rem] border border-slate-100 shadow-[0_40px_80px_-20px_rgba(0,0,0,0.06)] relative overflow-hidden">
                           ...
                           <div className="flex items-start justify-between mb-14">
                              <div className="space-y-4">
                                 ...
                                 <h2 className="text-4xl lg:text-5xl font-black text-earth-900 tracking-tighter">Change Password</h2>
                           ...
                           <form onSubmit={handleChangePassword} className="space-y-8 relative z-10">
                              <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                                 ...
                                 <div className="space-y-3">
                                    <label className="text-[10px] font-black uppercase tracking-[0.2em] text-slate-400 px-1">Password Strength</label>
                                    {getStrength(pwForm.newPw) ? (
                                       <div className={`h-[72px] flex items-center px-8 ${getStrength(pwForm.newPw).bg} rounded-3xl ${getStrength(pwForm.newPw).color} font-black text-[10px] uppercase tracking-widest gap-3 transition-all`}>
                                          <ShieldCheck size={16} /> Strength: {getStrength(pwForm.newPw).label}
                                       </div>
                                    ) : (
                                       <div className="h-[72px] flex items-center px-8 bg-slate-50 rounded-3xl text-slate-300 font-black text-[10px] uppercase tracking-widest gap-3">
         ...
         {isMenuModalOpen && (
            <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 bg-earth-900/60 backdrop-blur-md">
               <motion.div
                  ...
                  className="bg-white rounded-[4rem] p-12 max-w-2xl w-full shadow-[0_50px_100px_-20px_rgba(0,0,0,0.5)] relative overflow-hidden h-[90vh] overflow-y-auto"
               >
...
const DishCard = ({ dish, onEdit, onDelete, onFeature }) => {
   ...
   return (
      <div className="bg-white rounded-[3rem] overflow-hidden border border-slate-100 shadow-[0_20px_50px_-20px_rgba(0,0,0,0.05)] group hover:shadow-2xl transition-all duration-700 flex flex-col relative">
         <div className="h-64 relative overflow-hidden">
            <img src={dish.image} alt={dish.name} className="w-full h-full object-cover transition-transform duration-1000 group-hover:scale-110" />
         ...
         <div className="p-10 flex flex-col flex-grow">
            ...
            <h4 className="text-2xl font-black text-earth-900 tracking-tighter group-hover:translate-x-1 transition-transform duration-500 leading-none mb-2">{dish.name}</h4>
